...
class TestBitCenterLinearLayer(TestBitCenterLayer, TestCase):
    ...
    def get_input(self, n_train_sample, dim_in, dim_out, bias, cast_func=void_cast_func, do_double=True, seed=0):
        np.random.seed(seed)
        ...
        if do_double:
            input_delta = Parameter(torch.randn(n_train_sample, dim_in, dtype=torch.double).cuda(), requires_grad=True)
            input_fp = Parameter(torch.randn(n_train_sample, dim_in, dtype=torch.double).cuda(), requires_grad=True)
        else:
            input_delta = Parameter(cast_func(torch.randn(n_train_sample, dim_in, dtype=torch.double).cuda()), requires_grad=True)
            input_fp = Parameter(torch.randn(n_train_sample, dim_in, dtype=torch.float).cuda(), requires_grad=True)        
